...
=======
    ...
    const [isActiveForm, setIsActiveForm] = useState(false);
    const cart = useSelector(state => state.user.cart);
>>>>>>> 97560d035680a56d473e0b82f779a957cef3ae42
...
>>>>>>> 97560d035680a56d473e0b82f779a957cef3ae42
    ...
    function getPriceByChildNumber(item) {
        switch (item.child_number) {
            case 1:
                return {
                    ransom_amount_for_digital_photos: item.ransom_amount_for_digital_photos,
                    ransom_amount_for_calendar: item.ransom_amount_for_calendar,
                };
            case 2:
                return {
                    ransom_amount_for_digital_photos: item.ransom_amount_for_digital_photos_second,
                    ransom_amount_for_calendar: item.ransom_amount_for_calendar_second,
                };
            case 3:
                return {
                    ransom_amount_for_digital_photos: item.ransom_amount_for_digital_photos_third,
                    ransom_amount_for_calendar: item.ransom_amount_for_calendar_third,
                };
            default:
                // Мало ли, вдруг child_number будет 4 или 0?
                return {
                    ransom_amount_for_digital_photos: 0,
                    ransom_amount_for_calendar: 0,
                };
        }
    }
        ...
        fetchPhotoLineListWithTokenInterceptor(accessStor, '')
            .then(res => {
                if (isMounted && res.ok) {
                    res.json()
                        .then(data => {
                            console.log("photo_line_data", data)
                            // data[0].photo_theme.id
                            getNearestDate(data);
                            setlineLenght(data.length);
                            if (data.length > 0) {
...
=======
                                const {ransom_amount_for_digital_photos, ransom_amount_for_calendar}
                                    = getPriceByChildNumber(firstItem);

                                setPriceCalendar({
                                    ransom_amount_for_digital_photos: ransom_amount_for_digital_photos,
                                    ransom_amount_for_calendar: ransom_amount_for_calendar,
                                });
                            } else {
                                console.warn('Data is empty:', data);
                            }
            ...
            <div className={styles.orderWidggetWrap}>
                <div className={styles.orderWidggetContainer}>
                    ...
                    <AddKidsForm setIsActiveForm={setIsActiveForm} isActiveForm={isActiveForm} addBlock={addBlock}
                                 setModalActive={setModalActive} setModalText={setModalText}/>
                    ...
                    <div className={styles.orderPromoWrap}>
                        <div className={styles.orderPromoPromocode}>
                            <div className={styles.promoInputWrap}>
                                <input onChange={(e) => handlePromocodeChange(e)}
                                       className={true ? styles.promoInputActive : styles.promoInput}
